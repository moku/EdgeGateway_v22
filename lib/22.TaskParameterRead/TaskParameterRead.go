...
func Request(parameters *Parameters.Parameter) string {
	...
	if len(parameters.FlexibleTaskParameters()) > 0 {
		str := fmt.Sprintf(";%s=[%v]", Parameters.FlexibleTaskParameters, strings.Join(parameters.FlexibleTaskParameters()[:], ","))
		body += str
	}
	if len(parameters.StaticTaskParameters()) > 0 {
		str := fmt.Sprintf(";%s=[%v]", Parameters.StaticTaskParameters, strings.Join(parameters.StaticTaskParameters()[:], ","))
		body += str
	}
	body += "}"
