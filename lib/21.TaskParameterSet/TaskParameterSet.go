...
func Request(parameters *Parameters.Parameter) string {
	...
	fp := ""
	for k, v := range parameters.FlexibleTaskParameter() {
		fp += fmt.Sprintf("%s=%s;", k, strconv.Itoa(v))
	}
	fp = fp[:len(fp)-1] // remove the last semicolon
	body := fmt.Sprintf("{%s=%s;%s=%s;%s={%s}}", Parameters.MicroserviceID, parameters.MicroserviceID(),
		Parameters.TaskID, parameters.TaskID(),
		Parameters.FlexibleTaskParameter, fp)
